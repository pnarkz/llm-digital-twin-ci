...
"""
    ...
    @classmethod
    def get_prompts(cls, documents: list[CleanedDocument]) -> dict[DataCategory, list[dict]]:
        """
        CleanedDocument listesi alır, her birini GenerateDatasetSamplesPrompt'e çevirir
        ve dict formatında döner (ZenML ve Pydantic için uyumlu).
        """
        ...
        for category, category_documents in grouped_cleaned_documents.items():
            ...
            for document in category_documents:
                prompt_obj = cls.get_prompt(document.dict() if hasattr(document, "dict") else document)
                category_prompts.append(prompt_obj.dict())  # 🔥 dict formatında kaydet
    ...
    @classmethod
    def get_prompt(cls, document) -> GenerateDatasetSamplesPrompt:
        """
        CleanedDocument veya dict tipini destekleyen güvenli prompt üretici.
        """
        assert cls.prompt_template_str is not None, "Prompt template must be set before calling get_prompt()"

        # ✅ Her iki tip için uygun erişim yöntemi
        if isinstance(document, dict):
            data_category = document.get("category", "articles")  
            content = document.get("content", "")
        else:
            data_category = getattr(document, "category", "articles")  
        ...
        input_variables = {"extract": content}
    ...
    def generate(
        ...
        if mock:
            ...
            # 🔹 Sadece OpenAI'yi kullan (Hugging Face devre dışı)
            if settings.OPENAI_API_KEY:
                print("[INFO] Using OpenAI model:", settings.OPENAI_MODEL_ID)


                llm = ChatOpenAI(
                    model=settings.OPENAI_MODEL_ID or "gpt-4o-mini",
                    api_key=settings.OPENAI_API_KEY,
                    max_tokens=800,         # hız için sınırla
                    temperature=0.3,        # daha kararlı, az varyasyon
                    max_retries=3,          # retry azalsın
                    timeout=45,             # daha kısa timeout
                    request_timeout=60,     # fallback timeout
                )
        ...
        for category, category_prompts in prompts.items():
            ...
            batches = utils.misc.batch(langchain_category_prompts, size=2)

            flattened_instruct_dataset_samples = []
            for batch in batches:
                try:
                    batched_dataset_samples = chain.batch(batch, stop=None, config={"max_concurrency": 3})
